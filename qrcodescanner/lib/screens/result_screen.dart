  ...
  Widget _buildAIHeader(AppLocalizations l10n, double width) {
    return Row(
      children: [
        ...
        Text(l10n.aiSecurityAnalysisTitle, style: TextStyle(fontSize: width > 600 ? 20 : 18, fontWeight: FontWeight.bold)),
      ],
    );
  }
  ...
  Widget _buildAIAnalysisCard(ResultController controller) {
    return Container(
      ...
      child: controller.isLoading
          ? const Padding(padding: EdgeInsets.symmetric(vertical: 20), child: Center(child: CircularProgressIndicator(strokeWidth: 3)))
          : Text(controller.aiAnalysis, style: const TextStyle(fontSize: 15, height: 1.5, color: Colors.black87)),
    );
  }
        ...
        children: [
          Expanded(
            child: OutlinedButton.icon(
              onPressed: () {
                AnalyticsService.logEvent('result_action_copy');
                Clipboard.setData(ClipboardData(text: data));
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.copyButton)));
              },
          ...
          if (data.toLowerCase().startsWith("http")) ...[
            ...
            Expanded(
              child: ElevatedButton.icon(
                onPressed: () {
                  AnalyticsService.logEvent('result_action_open_url');
